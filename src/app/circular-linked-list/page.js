...
import CircularLinkedListVisualizer from "@/components/visualizers/CircularLinkedListVisualizer";
import CodePanel from "@/components/CodePanel";
import { circularLinkedListSnippets } from "@/lib/codeSnippets";
...
export default function CircularLinkedListPage() {
  ...
  const [activeCode, setActiveCode] = useState(circularLinkedListSnippets.declare);
  const [activeTitle, setActiveTitle] = useState("Declaration");
  const [error, setError] = useState("");

  function showCode(codeKey, title) {
    setActiveCode(circularLinkedListSnippets[codeKey]);
  ...
  return (
    <div className="p-8 max-w-6xl">
      <h2 className="text-2xl font-semibold mb-1">Circular Linked List</h2>
      <p className="text-sm text-[var(--color-text-muted)] mb-6">
        Same as Linked List, but the last node points back to Head.
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-5">
          <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 border-t-4 border-t-[var(--color-primary)] overflow-x-auto">
            <CircularLinkedListVisualizer nodes={nodes} />
          </div>
